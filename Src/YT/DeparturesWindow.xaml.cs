...
namespace YandexTrains
{
    ...
    public sealed partial class DeparturesWindow : Page
    {
        ...
        public DeparturesWindow()
        {
            ...
            _statusIndicatorTask = StatusIndicatorColorUpdate(
                _statusIndicatorTaskCancellationToken.Token);

            _loadingIndicatorTaskCancellationToken = new CancellationTokenSource();
            _loadingIndicatorTask = LoadingIndicatorUpdate(
                _loadingIndicatorTaskCancellationToken.Token);
        ...
        // OnDepartureBoardRefreshTimerTick
        private async void OnDepartureBoardRefreshTimerTick(object sender, object e)
        {
            await DepartureBoard.RefreshBoard();

        }//OnDepartureBoardRefreshTimerTick


        // OnSearchTextBoxKeyDown
        ...
        // StatusIndicatorColorUpdate
        private Task StatusIndicatorColorUpdate(CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                    ...
                    Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
                {
                    ...
                    Color statusColor = Color.FromArgb(
                        (byte)((255 / 2) 
                          * Math.Cos(0.1f * x) + (255 / 2)), // y = a * cos(f * x) + m
                        255,
                        255,
                        0
                    );
