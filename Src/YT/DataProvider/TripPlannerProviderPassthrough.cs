...
namespace YandexTrains.DataProvider
{
    ...
    public static class TripPlannerProviderPassthrough
    {
        ...
        private static bool IsAccessTokenExpired
        {
            get
            {
                return _tripPlannerProvider.IsAccessTokenExpired;
            }
        }
        ...
        public static async Task Initialize()
        {
            ...
            if (!File.Exists(_accessTokenFolder.Path + "\\CurrentAccessToken.txt") 
                || await IsFileEmpty())
            {
                _accessTokenFile = await _accessTokenFolder.CreateFileAsync
                (
                    "CurrentAccessToken.txt", CreationCollisionOption.ReplaceExisting
                );
            ...
            {
                _accessTokenFile = await _accessTokenFolder.GetFileAsync(
                    "CurrentAccessToken.txt");
            ...
            // TripPlannerProviderMock
            _tripPlannerProvider = new TripPlannerProviderMock(token)
            {
                // TripPlannerProviderMock
                Logger = new OutputLogger(nameof(TripPlannerProviderMock)) 
            };
        ...
        public static async Task<DepartureBoard> GetDepartureBoard(string stopId)
        {
            return await GetDepartureBoard(stopId, DateTime.Now);
        }

        public static async Task<DepartureBoard> GetDepartureBoard(
            string stopId, DateTime dateTime)
        {
            //if (IsAccessTokenExpired)
             //   await RenewToken();
        ...
        public static async Task<TripList> GetTripList(string originStopId,
            string destinationStopId)
        {
            return await GetTripList(originStopId,
                destinationStopId, DateTime.Now);
        }
            ...
            string destinationStopId, DateTime dateTime, bool isSearchForArrival = false)
        {
            ...
            return await _tripPlannerProvider.GetTripListAsync(originStopId,
                destinationStopId, dateTime, isSearchForArrival);
        }//GetTripList
        ...
        {
            IStorageFile storageFile = await _accessTokenFolder.GetFileAsync(
                "CurrentAccessToken.txt");
        ...
        private static string Serialize(AccessToken accessToken)
        {
            ...
            stringBuilder.AppendLine($"{nameof(accessToken.CreatedDateTime)}: " +
                $"{accessToken.CreatedDateTime.ToString("yyyyMMddHHmmss")}");

            stringBuilder.AppendLine($"{nameof(accessToken.ExpiresDateTime)}: " +
                $"{accessToken.ExpiresDateTime.ToString("yyyyMMddHHmmss")}");
            */
            ...
            foreach (string kvp in values)
            {
                ...
                switch (key)
                {
                    ...
                    case "CreatedDateTime":
                        accessToken.CreatedDateTime = DateTime.ParseExact(value,
                            "yyyyMMddHHmmss", 
                            System.Globalization.CultureInfo.InvariantCulture);
                        break;
                    case "ExpiresDateTime":
                        accessToken.ExpiresDateTime = DateTime.ParseExact(value,
                            "yyyyMMddHHmmss", 
                            System.Globalization.CultureInfo.InvariantCulture);
                        break;
                    default:
